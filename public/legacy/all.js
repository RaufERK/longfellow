...
function setCookie(name, value) {
  ...
  document.cookie =
    name + '=' + escape(value) + expires + path + domain + secure
}
...
function testit(id, price) {
  ...
  for (var i = 0; i < tmp_arr.length; i++) {
    ...
    sum =
      sum +
      subs.substring(0, subs.indexOf('*')) *
        subs.substring(subs.indexOf('*') + 1, subs.length)
  }
...
function recountw() {
  ...
  for (var i = 1; i < strok - 1; i++) {
    ...
    document.getElementById('summ' + i).value =
      document.getElementById('price' + i).value *
      document.getElementById('amount' + i).value
    document.getElementById('sweight' + i).value =
  ...
  if (document.getElementById('na_summu')) {
    document.getElementById('na_summu').value =
      document.getElementById('isumm').value + ' руб.'
  }
  document.getElementById('iweight').value = i3

  tmp_x =
    (document.getElementById('iweight').value -
      (document.getElementById('iweight').value % 3000)) /
    3000
  ...
  if (document.getElementById('fsumm')) {
    document.getElementById('fsumm').value =
      parseInt(document.getElementById('dsumm').value) +
      parseInt(document.getElementById('isumm').value)
  }
  //window.alert(tmp_x);
...
function del_one(it) {
  ...
  for (var i = 0; i < getIt('summ').length; i++) {
    if (i != it - 1) {
      the_cookie =
        the_cookie +
        document.all.id[i].value +
        '-' +
        document.all.amount[i].value +
        '*' +
...
function oformit() {
  disable_some(false)
  document.all.step2.dsumm.value =
    document.all.price[document.all.price.length - 1].value
...
function count_nal() {
  ...
  if (getIt('region').value == '') {
    ...
    if (getIt('region' + getIt('region').value)) {
      ...
      if (true) {
        // Для тестирования document.all.weight.length){
        strok = document.getElementById('order_table').rows.length
        //alert(strok);
        for (var i = 1; i < strok - 3; i++) {
          ...
          //alert(i);
          if (
            tmp_weight + getIt('we_' + i).value * getIt('am_' + i).value >
            1900
          ) {
      ...
      getIt('nal').value =
        (tmp_x * vel + 40) * 1.15 + getIt('isumm').value * 0.15 + 10 * kolvo
...
function getOrder() {
  ...
  if (!ordered) {
    ...
    if (getCookie('emailp'))
      document.getElementById('emailp').value = getCookie('emailp')
    if (getCookie('fnamep'))
      document.getElementById('fnamep').value = getCookie('fnamep')
    if (getCookie('cityp'))
      document.getElementById('cityp').value = getCookie('cityp')
    if (getCookie('adresp'))
      document.getElementById('adresp').value = getCookie('adresp')
    if (getCookie('phonep'))
      document.getElementById('phonep').value = getCookie('phonep')
    if (getCookie('phone2p'))
      document.getElementById('phone2p').value = getCookie('phone2p')
    if (getCookie('primp'))
      document.getElementById('primp').value = getCookie('primp')
    if (getCookie('indeks'))
      document.getElementById('indeks').value = getCookie('indeks')
    if (getCookie('region'))
      document.getElementById('region').value = getCookie('region')
    ...
    if (getCookie('adres'))
      document.getElementById('adres').value = getCookie('adres')
    if (getCookie('phone'))
      document.getElementById('phone').value = getCookie('phone')
    if (getCookie('phone2'))
      document.getElementById('phone2').value = getCookie('phone2')
    if (getCookie('prim'))
      document.getElementById('prim').value = getCookie('prim')
    //}
    if (tmp != null && tmp != '') {
...
function RecountSum() {
  ...
  if (
    document.getElementById('dsumm') &&
    document.getElementById('delivery1').checked == true
  ) {
    if (tmp_x < 3) {
      if (tmp_x == 1) {
        document.getElementById('dsumm').value = '200'
  ...
  if (
    getIt('region' + getIt('region').value) &&
  ...
  if (document.getElementById('dsumm2')) {
    document.getElementById('dsumm2').value = parseInt(
      document.getElementById('dsumm').value
    )
  }
}
...
function goforit(num) {
  ...
  if (num == 2) {
    ...
    setCookie(
      'fnamep',
      document.getElementById('fnamep').value,
      cookieDate,
      '/'
    )
    setCookie(
    ...
    setCookie(
      'adresp',
      document.getElementById('adresp').value,
      cookieDate,
      '/'
    )
    setCookie(
    ...
    setCookie(
      'phone2p',
      document.getElementById('phone2p').value,
      cookieDate,
      '/'
    )
...
function setcart() {
  //устанавливает значение cart2=содержимому обновленной корзины
  var tmp_cart = ''
...
function setCartCokie() {
  //устанавливает значение cookie("cart")=содержимому обновленной корзины
  var tmp_cart = ''
...
function addDays(date, n) {
  // добавляет n дней к указанной дате
...
// Redirect legacy "Помощь" button/form to /help
;(function () {
  ...
  function setup() {
    ...
    if (btn) {
      if (btn.addEventListener)
        btn.addEventListener('click', redirectToHelp, false)
      else if (btn.attachEvent) btn.attachEvent('onclick', redirectToHelp)
    }
    var form = document.getElementById('help_form')
    if (form) {
      if (form.addEventListener)
        form.addEventListener('submit', redirectToHelp, false)
      else if (form.attachEvent) form.attachEvent('onsubmit', redirectToHelp)
    }
  }
  if (
